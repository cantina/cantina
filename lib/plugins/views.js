...
views.attach = function(options) {
  ...
  // Initialize the app.views namespace.
  app.views = app.views || {};
  ...
  if (!path.existsSync(path.resolve(app.root, options.path, options.layout + '.' + options.ext))) {
    ...
    if (options.layout === 'layout') {
      ...
      throw new Error('The layout does not exist (' + options.layout + ').');
    }
  }

  // Load the view helpers plugin.
  app.use(require('./views/views-helpers'), options);
  ...
  function render(view, opts, cb) {
    ...
    app.views.processHelpers.call(self, opts, function(err) {
