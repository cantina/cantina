...
Cantina.prototype.use = function(plugin, conf) {
  ...
  // Ensure plugin conf namespace exists.
  this.conf[plugin.name] = this.conf[plugin.name] || {};

  // Merge conf
  if (conf) {
    deepExtend(this.conf[plugin.name], conf);
  }
 ...
 *     app.configure({myplugin: {}});
 */
Cantina.prototype.configure = function(name, newConf) {
  var conf = {};
  if (newConf) {
    conf[name] = newConf;
  }
...
Cantina.prototype.initPlugin = function(plugin, done) {
  ...
  plugin.init(app.conf[plugin.name], imports, function(err, provided) {
