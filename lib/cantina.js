...
Cantina.prototype.init = function(callback) {
  ...
  tasks = Object.keys(app.plugins).map(function(name) {
    return app.initPlugin.bind(app, app.plugins[name]);
  });
...
/**
 * Call plugin ready handlers.
 */
Cantina.prototype.ready = function(callback) {
  var app = this, tasks;
...
Cantina.prototype.initPlugin = function(plugin, done) {
  ...
  // Gather imports for this plugin.
  if (plugin.imports) {
    Object.keys(plugin.imports).forEach(function(name) {
      imports[name] = app[name];
    });
  }

  // Initialize the plugin.
  plugin.init(app.conf.get(plugin.name), imports, function(err, exported) {
    // Bind error handler if it exists.
    if (plugin.error) {
      app.on('error', plugin.error);
    }

    if (err) return done(err);

    if (exported) {
      app[plugin.name] = exported;
    }
...
Cantina.prototype.checkPlugin = function(plugin) {
  ...
  if (plugin.imports) {
    Object.keys(plugin.imports).forEach(function(name) {
      ...
      if (!semver.satisfies(app.plugins[name].version, plugin.imports[name])) {
